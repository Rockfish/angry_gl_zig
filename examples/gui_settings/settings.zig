...
const GameSettings = struct {
    ...
    pub fn get_monster_y(self: *const Self) f32 {
        return self.player_model_scale * self.player_model_gun_height;
    }

    pub fn getLightColor(self: *Self) Vec3 {
        return vec3(self.non_blue * 0.406, self.non_blue * 0.723, 1.0).mulScalar(self.light_factor * 1.0);
    }

    pub fn getLightDirection(self: *Self) Vec3 {
        return self.light_direction.normalize();
    }
    ...
    pub fn getAmbientColor(self: *Self) Vec3 {
        return vec3(self.non_blue * 0.7, self.non_blue * 0.7, 0.7).mulScalar(self.light_factor * 0.10);
    }

    pub fn getFloorLightColor(self: *Self) Vec3 {
        return vec3(self.floor_non_blue * 0.406, self.floor_non_blue * 0.723, 1.0).mulScalar(self.floor_light_factor * 1.0);
    }

    pub fn getFloorAmbientColor(self: *Self) Vec3 {
        return vec3(self.floor_non_blue * 0.7, self.floor_non_blue * 0.7, 0.7).mulScalar(self.floor_light_factor * 0.50);
    }
};

pub fn getSettings(allocator: Allocator) !GameSettings {
    const file = try std.fs.cwd().openFile("examples/gui_settings/settings.toml", .{});
    const toml = try file.readToEndAlloc(allocator, 256 * 1024);
    defer file.close();
    defer allocator.free(toml);

    const settings = try tomlz.decode(GameSettings, allocator, toml);
